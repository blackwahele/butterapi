...
export const ManageUserMiddleware = async function(req, res, next) {
    const reqObj = {
    ...
    try {
        if(!req.body){
            return res.status(500).json(new ApiError(500,"User List Request!", "Empty Request body."));
        };
        ...
        return res.status(500).json(new ApiError(500,"User List Request!", error?.message,error));
